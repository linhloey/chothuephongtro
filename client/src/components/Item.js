...
const Item = ({images, user, title, star, description, attributes, overview, id}) => {
    ...
    <div className='w-full flex border-t border-orange-600 py-3'>
        <Link 
            to={`/chi-tiet/${formatVNToString(title)}/${id}`} 
        ...
        >
            {images.length > 0 && images.filter((i, index) => indexs.some(i => i === index)).map((i, index) => {
                ...
                <span className='bg-overlay-70 text-sm text-white px-2 rounded-md absolute left-1 bottom-1'>{`${images.length} anh`}</span>
                <span 
        ...
        <div className='w-3/5'>
            ...
            <div className='flex items-center my-5 justify-between'>
                ...
                <div className='flex items-center gap-1'>
                    <button
                        type='button'
                        className='bg-blue-700 whitespace-nowrap text-white p-1 rounded-md'
                    >
                        {`Goi ${user?.phone}`}
                    </button>
                    <button
